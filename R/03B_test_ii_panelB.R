# =============================================================================
# 03B_test_ii_panelB.R
# Test (ii): Domain-label permutation — Panel B (FE: target + skill_name)
# 1000 réplicas. SESIÓN LIMPIA. Cerrar R al terminar.
# Misma lógica que 03A. Usa mismo seed por réplica → misma permutación de
# skills → permite comparación directa de concordancia entre paneles.
# REQUIERE: test_ii_panelA.rds (correr 03A primero)
# =============================================================================
...
B_PLACEBO <- 1000L
ckpt_file <- file.path(output_dir, "rds", "test_ii_panelB_ckpt.rds")
...
for (b in seq(b_start, B_PLACEBO)) {
  if (b %% 50 == 0) message("Test ii Panel B: ", b, "/", B_PLACEBO)

  # Mismo seed que Panel A → misma permutación de skills en réplica b
  set.seed(SEED * 1000L + b)

  perm_domains <- sample(skill_domain_tbl$domain)
  perm_map <- data.table(
    skill_name  = skill_domain_tbl$skill_name,
    domain_perm = perm_domains
  )
...
# Consolidar Test ii completo
perm_ii <- rbind(
  readRDS(file.path(output_dir, "rds", "test_ii_panelA.rds")),
  rbindlist(results)
)
saveRDS(perm_ii, file.path(output_dir, "rds", "test_ii_domain_perm.rds"))
fwrite(perm_ii,  file.path(output_dir, "tables", "table_SI_test_ii_raw.csv"))
if (file.exists(ckpt_file)) file.remove(ckpt_file)
message("Test ii completo (", 2 * B_PLACEBO, " modelos totales). CERRAR R.")

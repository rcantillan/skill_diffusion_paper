# =============================================================================
# 04A_test_iii_panelA.R
# Test (iii): Within-stratum permutation — Panel A (FE: source + skill_name)
# 1000 réplicas. SESIÓN LIMPIA. Cerrar R al terminar.
#
# QUÉ TESTEA:
#   Permuta el outcome `diffusion` dentro de celdas (domain × wage_q_source).
#   Esto destruye la asociación entre la dirección del movimiento salarial
#   (wage_up, wage_down) y la adopción del skill, PRESERVANDO:
#     - Tasa de difusión promedio por dominio de skill
#     - Tasa de difusión promedio por quintil salarial de source
#     - La composición de skills dentro de cada celda
#   Si Theta_up y Theta_dn colapsan, la señal ATC es genuinamente
#   direccional y no un artefacto de la composición de los estratos.
#
# ESTRUCTURA TRIÁDICA:
#   La permutación actúa al nivel de tríada individual (j, i, s) dentro
#   de la celda. Esto NO preserva perfectamente la dependencia entre
#   tríadas de la misma díada (j,i) — la tríada (j,i,s1) puede recibir
#   el outcome de (j',i',s2) con j≠j' o i≠i'. Esto hace el test
#   CONSERVADOR: si los coeficientes colapsan de todas formas, la
#   evidencia es más fuerte, no más débil.
#   Reportar en SI: "permutation was performed at the observation level
#   within strata (domain × source wage quintile), a conservative design
#   that does not fully preserve dyadic clustering."
#
# wage_q_source:
#   Quintil de mean(wage_gap) por source occupation — calculado en el setup
#   como atributo de la source, no de la díada. Verificado con stopifnot
#   en el setup. Ver 00_setup_comun.R para la justificación.
#
# SESIÓN LIMPIA COMPLETAMENTE SEPARADA de 04B. Cerrar R al terminar.
# =============================================================================
...
B_PLACEBO <- 1000L
ckpt_file <- file.path(output_dir, "rds", "test_iii_panelA_ckpt.rds")

# Verificar wage_q_source (debe ser atributo de source, calculado en setup)
stopifnot("wage_q_source debe existir en dt_sample" =
            "wage_q_source" %in% names(dt_sample))
check_q <- dt_sample[, .(n_q = uniqueN(wage_q_source)), by = source]
stopifnot("Cada source debe tener un único quintil" = all(check_q$n_q == 1))
rm(check_q)

message("Distribución de celdas (domain × wage_q_source):")
cell_dist <- dt_sample[, .N, keyby = .(domain, wage_q_source)]
print(cell_dist)
message("Obs mínimas por celda: ", min(cell_dist$N))
message("Obs máximas por celda: ", max(cell_dist$N))
...
if (file.exists(ckpt_file)) {
  ...
  cat("Solo válido si fue generado con ESTE script (seeds por réplica).\n")
  cat("Si viene de versión anterior, borrarlo:\n")
  cat("  file.remove('", ckpt_file, "')\n", sep = "")
  cat("====================================================\n\n")
  confirm <- readline("¿Confirmas compatibilidad? (si/no): ")
...
for (b in seq(b_start, B_PLACEBO)) {
  message("Test iii Panel A | b=", b, "/", B_PLACEBO,
          " | ", format(Sys.time(), "%H:%M:%S"))

  set.seed(SEED * 1000L + b)

  # Permutar diffusion dentro de (domain × wage_q_source)
  dt_sample[, diff_orig := diffusion]
  dt_sample[, diffusion := sample(diffusion), by = .(domain, wage_q_source)]
  ...
  rm(m); gc(); gc()
  dt_sample[, diffusion := diff_orig]
  dt_sample[, diff_orig := NULL]
...
saveRDS(rbindlist(results),
        file.path(output_dir, "rds", "test_iii_panelA.rds"))
if (file.exists(ckpt_file)) file.remove(ckpt_file)
message("Test iii Panel A completo (", B_PLACEBO, " réplicas). CERRAR R.")

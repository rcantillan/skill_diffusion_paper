# =============================================================================
# 04B_test_iii_panelB.R
# Test (iii): Within-stratum permutation — Panel B (FE: target + skill_name)
# 1000 réplicas. SESIÓN LIMPIA. Cerrar R al terminar.
# Usa mismo seed por réplica que 04A → misma permutación de diffusion.
# REQUIERE: test_iii_panelA.rds (correr 04A primero)
# =============================================================================
...
B_PLACEBO <- 1000L
ckpt_file <- file.path(output_dir, "rds", "test_iii_panelB_ckpt.rds")
...
for (b in seq(b_start, B_PLACEBO)) {
  if (b %% 50 == 0) message("Test iii Panel B: ", b, "/", B_PLACEBO)

  # Mismo seed que Panel A → misma permutación de diffusion en réplica b
...
# Consolidar Test iii completo
perm_iii <- rbind(
  readRDS(file.path(output_dir, "rds", "test_iii_panelA.rds")),
  rbindlist(results)
)
saveRDS(perm_iii, file.path(output_dir, "rds", "test_iii_stratum_perm.rds"))
fwrite(perm_iii,  file.path(output_dir, "tables", "table_SI_test_iii_raw.csv"))
if (file.exists(ckpt_file)) file.remove(ckpt_file)
message("Test iii completo (", 2 * B_PLACEBO, " modelos totales). CERRAR R.")
